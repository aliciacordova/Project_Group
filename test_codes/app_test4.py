...
def index():
    ...
    # Load data from MongoDB to build the images and Graphs
    ...
    # 6. Call the function to Export Chart to AWS
    exportAWS(image_name)
    
    # BUILD THE PRICE HISTORY GRAPH
    # 1. Filter transaction and price history
    ...
    # 2. Create image container and elements
    ...
    # 1. Filter the dataframe for relevant transaction types
    ...
    G = nx.MultiDiGraph()
    # 6. Create an empty dictionary for the edge labels and create them
    mylabels={}
    for row in range(len(deals_df)):
        # Add to-from nodes
    ...
    # 7. Define network chart type as circular
    pos=nx.circular_layout(G)
    # 8. Define degrees for use in displaying node sizes
    d = dict(G.degree)
    ...
    # 10. Save the image locally
    image_name = "network_graph.png"
    plt.savefig("static/images/" + image_name)
    # 11. Call the function to Export Chart to AWS
    exportAWS(image_name)
    
    
    # BUILD THE PUNK FACTS TABLE
